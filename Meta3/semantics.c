    ...
    if((sym_aux=get_sym(funcDec,st_root))!=NULL){ //se já existir declarada
        ...
        while(paramDec!=NULL){ //iterate through paramList childs
            ...
            if(paramDec->child->next!=NULL){
                ...
                else add_sym(funcDefTable,create_sym(paramDec->child->next->tk->value,str_to_type(paramDec->child->str),0,1));            
            }
            add_param(funcDec,str_to_type(paramDec->child->str)); //add paramtype to param list of funcDec symbol
        ...
        if( (sym_aux = get_sym(s_aux0,st_root)) != NULL) delete_sym_from_table(sym_aux->name,st_root);
...
void handle_funcDefs(node* n) {
    ...
    if((funcDefTable=get_sym_table(funcName))!=NULL){//SE JÁ EXISTIR TABELA DE SÍMBOLOS PARA ESTA TABELA É PQ ELA JÁ FOI DECLARADA OU DEFINIDA ANTERIORMENTE
        ...
        if(!flag) add_funcBody_syms_to_table(funcDefTable,aux); //<-esta funcao deve fazer a analise semantica destes erros^
        
        /***********************************************************************************************************************/
        return;
    } else { //se ainda n tiver sido declarada ou definida
        s_aux0=create_sym(funcName,retType,0,0);
        if((sym_aux=get_sym(s_aux0,st_root))!=NULL) delete_sym_from_table(sym_aux->name,st_root); //delete var from table to be replaced with funcdefinition

        if(!flag) add_sym(st_root,funcDef); //add to global table 
...
int check_params_list_types(sym *sym_defined, sym *sym_declared) {
    ...
    while(list0!=NULL && list1!=NULL){
        if(list1->type!=list0->type) return 0; //different param types
        list0=list0->next;
        list1=list1->next;
    }
    if (list1==NULL && list0==NULL) return 1;
    else return 0;
}
...
int paramsCounter(struct param* param_list, int flag) {
    ...
    while (param_list_aux) {
        ...
        else counter++; //conta voids nos parametros
...
_type get_statement_type(node* statement, sym_table *st) {
    ...
    if (strcmp(statement->str,"Plus")==0 || strcmp(statement->str,"Minus")==0) {
        ...
        if(checkConflitingTypes(intlit,t_aux)||(strcmp(statement->child->str,"Call")!=0 && statement->child->param_list!=NULL)){
    ...
    else if(strcmp(statement->str,"Null")==0) return voidlit;
    else if(isTerminal(statement)) return getTerminalType(statement,st); //intlit ou reallit ou undef ou id..
    else return undef;
}
...
void add_stat_decs_syms_to_table(sym_table* st, node* stats_decs) {
    ...
    while(stat_dec!=NULL){
        if(strcmp(stat_dec->str,"Declaration")==0){
            ...
            if(!isDeclared(s,st)){
                if(!flag) add_sym(st,s); //adiciona sym à table  
            } else {
                printf("Line %d, col %d: Symbol %s already defined\n", stat_dec->child->next->tk->lineNum, stat_dec->child->next->tk->colNum, stat_dec->child->next->tk->value);
            }
            if(flag) { 
                printf("Line %d, col %d: Invalid use of void type in declaration\n",stat_dec->child->next->tk->lineNum,stat_dec->child->next->tk->colNum); 
                flag=0; free_sym(s);
            }
        }
        else expr_type=get_statement_type(stat_dec, st);
...
_type get_operation_type(node * operation,sym_table *st){
    ...
    if(n_aux->param_list!=NULL||n_aux->next->param_list!=NULL) {
        ...
        return reallit;
    } else if ((type0==shortlit && type1==shortlit) || (type0==shortlit && type1==charlit) || (type0==charlit && type1==shortlit)) {
...
_type getTerminalType(node *n,sym_table *st) {
    ...
    if(strncmp(n->str,"Id",2)==0){
        ...
        if((aux0=get_sym(aux1,st))==NULL){ //if not in local table, search in global table
            ...
            } else n->type=aux0->type; free_sym(aux1); return aux0->type; 
        } else {
            n->type=aux0->type;
            free_sym(aux1);
            return aux0->type;
    ...
    else if(strncmp(n->str,"ChrLit",6)==0) { n->type=intlit; return intlit; }
    else if(strncmp(n->str,"IntLit",6)==0) { n->type=intlit; return intlit; }
    else if(strncmp(n->str,"RealLit",7)==0) { n->type=reallit; return reallit; }
    else return undef;
}

int isTerminal(node *n) {
    if(strncmp(n->str,"Id",2)==0) return 1;
    else if(strncmp(n->str,"ChrLit",6)==0) return 1;
    else if(strncmp(n->str,"IntLit",6)==0) return 1;
    else if(strncmp(n->str,"RealLit",7)==0) return 1;
    else return 0;
}
...
int checkConflitingTypes(_type expectedType,_type gotType){
    ...
    if(aux0==aux1) return 0; //no conflict
    else if(aux0==reallit && aux1==intlit) return 0; //no conflict
    else return 1;
}