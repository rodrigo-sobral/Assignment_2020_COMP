...
void get_funcDefs_code(node *n){ //FUNC DEFS
    ...
    printf("\ndefine %s @%s(",type_to_llvm(t),aux->next->tk->value);
...
void print_funcBody_code(node* funcBody){
    ...
    while(funcDecAndStats){
        if(strcmp(funcDecAndStats->str,"Declaration")==0){
            ...
            //STATEMENTS
            handle_statements(funcDecAndStats);
        }
...
void handle_statements(node* statement){
    ...
    if(statement!=NULL){
        ...
        if(strcmp(statement->str,"Plus")==0){
            //1 nó filho
            handle_statements(statement->child);
        ...
        //COMMA
            ...
            handle_statements(statement->child);
            handle_statements(statement->child->next);
        ...
        //RETURN
            ...
            t=statement->next->type;
            retFlag=1; //set retFlag
            if(strcmp(statement->next->str,"Null")==0){
                printf("\tret void\n");
            }
            else{
                handle_statements(statement->next);
                printf("\tret %s %s", type_to_llvm(t),statement->child->llvm_name);
